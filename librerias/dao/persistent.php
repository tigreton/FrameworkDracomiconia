				...
				foreach($this->entity->fields as $field)
					if($field instanceof DateField)
						...
						if(!isset($row[$field->name]))
							throw new NotExistException('Field "'.$field->name.'" do not exists in table "'.$name.'"');
	...
	function loadData($row){
		foreach($this->entity->fields as $field)
			if($field instanceof DateField)
				...
				if($row[$field->name])
					$this->fields[$field->name]=unserialize(stripslashes($row[$field->name]));
				...
				if(!isset($row[$field->name])){
					throw new NotExistException('Field "'.$field->name.'" do not exists in table "'.$this->entity->name.'"');
				}
				else $this->fields[$field->name]=$row[$field->name];
		$this->id=$row['id'];
	}
